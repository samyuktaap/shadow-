...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === 'DATA_SHADOW_ANALYSIS') {
    const { riskLevel, cookieCount, dangerousFields, trackerNames, domain } = message.data;
    ...
    chrome.storage.local.get('shieldActive', (data) => {
      if (data.shieldActive) {
        ...
        showRedAlert(cookieCount, dangerousFields, trackerNames, domain);
      }
    });
  }
});
...
function showRedAlert(count, fields, names, domain) {
  ...
  wrapper.innerHTML = `
    ...
    <div class="ds-header">
      <span class="ds-warning-icon">⚠️</span> RED ALERT
    </div>
    <div class="ds-body">
      This site is accessing <b>${count} shadow trackers</b>. 
      <div class="ds-fields">Danger: ${alertText}</div>
      ${trackerList}
      Your privacy is being compromised in short.
